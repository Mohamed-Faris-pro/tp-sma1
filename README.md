...
* Envoi de payloads JSON via Postman vers `https://api.openai.com/v1/chat/completions`.
...
* Utilisation de modèles comme `Llama 3` ou `Mistral` directement sur la machine.
...
Exploration de l'inférence ultra-rapide avec l'architecture LPU de Groq.

* Intégration du SDK Groq.
* Comparaison de la vitesse de génération par rapport aux autres fournisseurs.
...
Utilisation de modèles génératifs (DALL-E) pour la création visuelle.
...
* **Exemple :** Dans une critique de restaurant, identifier le sentiment pour la "Nourriture" vs le "Service".
* Utilisation de prompts structurés pour obtenir une sortie en format JSON.
...
3. Configurez vos clés dans un fichier `.env` (non inclus dans le dépôt).
4. Installez les dépendances :
```bash
pip install tiktoken openai groq python-dotenv
...
### 💡 Rappel technique pour tes captures

* Crée un dossier `assets` dans ton projet.
* Enregistre tes images dedans avec des noms simples (ex: `01.png`).
* N'oublie pas : `git add assets/`, `git commit -m "Ajout des preuves visuelles"`, puis `git push`.
